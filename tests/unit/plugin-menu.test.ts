		...
		root!.render(
			createElement(
				...
				createElement(TopBar, {
					...
					onOpenPanel: () => {},
					onOpenSettings: () => {},
					onManagePlugins: () => managedPlugins++,
					onOpenBgTasks: () => {},
					onOpenGlobalSearch: () => {},
...
describe("插件面板的内容", () => {
	...
	it("使用图钉按钮，并可用方向键调整插件顺序", () => {
		const { container, sent } = mount(
			[hostEntry("host:plugins"), viewEntry("mail", "Mailbox", false), viewEntry("notes", "Notes", false)],
			[],
			[
				{ id: "mail", name: "Mailbox" },
				{ id: "notes", name: "Notes" },
		...
		act(() => pluginTrigger(container)!.click());
		expect(pinOf(rows()[0])!.querySelector("svg")).toBeTruthy();
		const handle = rows()[1].querySelector<HTMLButtonElement>(".pm-drag")!;
		...
		expect(sent.filter((m) => m.type === "set_settings")).toEqual([
			{ type: "set_settings", uiLayout: { order: ["notes:__view", "mail:__view"] } },
		]);
	});

	it("底部“管理插件”关闭面板并打开插件管理", () => {
		const result = mount([hostEntry("host:plugins")]);
		act(() => pluginTrigger(result.container)!.click());
		act(() => panel()!.querySelector<HTMLButtonElement>(".pm-manage")!.click());
		expect(panel()).toBeNull();
		expect(result.managedPlugins).toBe(1);
	});
