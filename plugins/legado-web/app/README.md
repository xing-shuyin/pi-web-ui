...
- **章末导航**（读到底就能翻章）：正文末尾一条「← 上一章 / 目录 / 下一章 →」（带《书名》· 第 n/总 章），读完这一章不用滚回顶部；换章后自动回顶，第一章「上一章」、最后一章「下一章」置灰并写明「已是最后一章」（顶栏同名按钮同规则），章末「目录」展开目录并回顶。判断逻辑在 `src/core/chapnav.ts`（纯函数，有单测）
